  ...
  return (
    <div className="min-h-screen py-5 sm:py-8 px-3 sm:px-4 pb-28 sm:pb-24 animate-in fade-in duration-500">
      <div className="mx-auto max-w-2xl">
        ...
        <div className="mb-4 sm:mb-6 rounded-2xl border border-neutral-200 bg-white p-4 sm:p-6 md:p-8 shadow-sm animate-in slide-in-from-left duration-500 delay-100">
          <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-[Playfair_Display] font-semibold text-gray-900">
            Order Summary
          </h2>
          ...
          <div className="space-y-2.5 sm:space-y-3">
            {cartData.cartItems.map((item, index) => (
              ...
              >
                <span className="text-gray-700 min-w-0 mr-2 truncate">
          ...
          <div className="mt-3 sm:mt-4 bg-[#f0ebe2] rounded-xl p-3 sm:p-4">
        ...
        <div className="mb-4 sm:mb-6 rounded-2xl border border-neutral-200 bg-white p-4 sm:p-6 md:p-8 shadow-sm animate-in slide-in-from-left duration-500 delay-200">
          <h2 className="mb-3 sm:mb-4 text-base sm:text-lg font-[Playfair_Display] font-semibold text-gray-900">
            Shipping Address
          </h2>
          <div className="space-y-0.5 sm:space-y-1 text-sm text-gray-700">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-neutral-200 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] px-3 sm:px-4 py-2.5 sm:py-3 md:px-8">
        <div className="flex items-center gap-2.5 sm:gap-3 max-w-3xl mx-auto">
          <button
            onClick={() => router.back()}
            className="flex-1 border border-neutral-300 text-neutral-600 hover:border-neutral-400 rounded-xl h-11 sm:h-12 bg-white font-medium transition-all cursor-pointer text-sm sm:text-base"
          >
            ← Back
          </button>
          <button
            onClick={handlePay}
            disabled={loading}
            className="flex-[2] flex items-center justify-center gap-2 bg-[#3d6b4f] hover:bg-[#2f5540] text-white rounded-xl h-11 sm:h-12 font-medium transition-all disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer text-sm sm:text-base"
          >
            {loading ? (
              <>
              ...
              <span className="truncate">Pay ₹{grandTotal.toLocaleString("en-IN")} Securely</span>
            )}
          </button>
        </div>
      </div>
